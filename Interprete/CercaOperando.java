	...
	public void Cerca(String linea, int i, HashMap<String, Long> variabili) {
		if (Character.isDigit(linea.charAt(i))) {
			...
			if (Character.isAlphabetic(linea.charAt(i))) {
				System.out.println(
						"Errore nella sintassi del'espressione: al valore della variabile c'è attaccata una lettera");
				System.exit(0);
			}
			...
			if (nome_variabile.contentEquals("ADD") || nome_variabile.contentEquals("SUB")
					|| nome_variabile.contentEquals("MUL") || nome_variabile.contentEquals("DIV")) {
				System.out
						.println("Errore nella sintassi dell'espressione: manca parentesi aperta prima dell'operatore");
				System.exit(0);
			}
